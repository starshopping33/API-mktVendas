...
namespace API_mktVendas.Application.Service
{
    ...
    {
            public async Task RegisterAsync(string nome, string cpf, string email,string telefone, DateTime datanascimento , string password)
            {
        ...
        public async Task<string> LoginAsync(string email, string password)
        {
            ...
            return GenerateToken(
                  ...
                  user.Id,user.Email,user.Cpf, user.Telefone,user.Nome,user.ImagemBase64, user.DataNascimento ,user.IsAdmin);
        }

        private string GenerateToken(int id ,string email, string cpf,string telefone,string nome, string imagebase64,DateTime datanascimento ,bool isAdmin)
        {
            ...
            var claims = new[]
            {
                ...
                new Claim("imagebase64", imagebase64),
                new Claim("datanascimento", datanascimento.ToString("yyyy-MM-dd")),
                new Claim("nome", nome),
        ...
        public async Task<Usuario?> UpdateUsuarioAsync(int id, UpdateDto dto)
        {
            ...
            if (!string.IsNullOrEmpty(dto.ImagemBase64))
            {
                ...
                try
                {
                    Convert.FromBase64String(dto.ImagemBase64);
                    usuario.ImagemBase64 = dto.ImagemBase64;
                }
                catch
                {
                    throw new ArgumentException("Imagem inválida: a string Base64 não está em um formato correto.");
                }
            }
